...
class Cargo {
  constructor(name, status, origin, destination, departureDate) {
    ...
    this.name = name;
    (this.status = status), (this.origin = origin);
    ...
    for (const key in cargoList[i]) {
      ...
      if (key === "status") {
        ...
        select
          ...
          [field[key] - 1].setAttribute("selected", "");
...
export function addNewCargo() {
  ...
  const cargoDepartureDate =
    document.getElementById("cargoDepartureDate").value;
...
document.querySelector(".cargo-list").addEventListener("change", (event) => {
  ...
  if (selectedValue === "3") {
    ...
    if (today < departureDate) {
        document.querySelector(".table__error").innerHTML =
          "Дата отправления груза еще не наступила. Груз не может быть в статусе 'Доставлен'!";
        const currentStatus = cargoList[index].status;
